        ...
        async with streamable_http_client(url, http_client=client) as (read, write, _):
            async with ClientSession(read, write) as session:
                ...
                for context in (0, 1, 3):
                    result = await session.call_tool('search_text', {'query': '唯讀連線測試成功',
                        'root_id': root_id, 'context_lines': context})
                    if result.isError:
                        raise ValueError('前後文契約驗收失敗。')
                batch = await session.call_tool('read_files', {'root_id': root_id,
                    'files': [{'path': 'README.md'}, {'path': '../denied.txt'}]})
                if batch.isError:
                    raise ValueError('批次契約驗收失敗。')
                batch_data = json.loads(batch.content[0].text)['files']
                if 'content' not in batch_data[0] or 'skipped_reason' not in batch_data[1]:
                    raise ValueError('批次逐項驗證失敗。')
                searched = await session.call_tool('search_texts', {'root_id': root_id,
                    'queries': ['唯讀連線測試成功', 'REMOTE_SMOKE_ABSENT_93817']})
                if searched.isError:
                    raise ValueError('批次搜尋驗收失敗。')
