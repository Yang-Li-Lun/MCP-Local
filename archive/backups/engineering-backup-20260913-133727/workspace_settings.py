...
def normalize_workspace(roots: list, default_root: str, settings: dict | None = None) -> dict:
    ...
    for item in roots:
        ...
        for previous in result:
            ...
            if reader.root.is_relative_to(other) or other.is_relative_to(reader.root):
                raise ValueError('共享資料夾不可重複或互相包含，請改用彼此獨立的專用資料夾。')
        result.append({'id': identifier, 'name': name.strip(), 'path': str(reader.root),
