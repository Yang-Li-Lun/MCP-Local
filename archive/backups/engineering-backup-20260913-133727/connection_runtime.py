            ...
            for stage, command in zip(('建立設定', '檢查連線', '通道程序執行中'), commands):
                with self.lock:
                    ...
                    process = subprocess.Popen(
                        [sys._base_executable, '-c', wrapper, *command],
                ...
                if code:
                    ...
                    diagnostic = b''.join(output).lower()
                    if any(word in diagnostic for word in (b'401', b'403', b'unauthorized', b'credential')):
                        self.error_kind = ConnectionErrorKind.AUTH_FAILED
                    hint = ('請檢查金鑰及通道授權。' if any(word in diagnostic for word in
                            (b'401', b'403', b'unauthorized', b'credential')) else
                            '請確認金鑰、通道識別碼及網路連線。')
