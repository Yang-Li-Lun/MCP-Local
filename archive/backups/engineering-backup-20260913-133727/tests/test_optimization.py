...
class OptimizationTests(unittest.TestCase):
    ...
    def test_background_permanent_key_failure_and_retry(self):
        import autostart
        from unittest.mock import Mock
        import queue
        settings = {'auto_start': True, 'auto_connect': True, 'settings_version': 3}
        for key, expected in [('', 'KEY_MISSING'), (ValueError('private-data'), 'KEY_DECRYPT_FAILED')]:
            logger = Mock()
            connection = Mock(thread=None)
            store = Mock()
            if isinstance(key, Exception):
                store.load.side_effect = key
            else:
                store.load.return_value = key
            with patch('tray_windows.kernel') as kernel, patch('autostart.ctypes.get_last_error', return_value=0), \
                    patch('autostart.create_logger', return_value=logger), \
                    patch('autostart.load_settings', return_value=settings), \
                    patch('autostart.build_commands'), patch('autostart.KeyStore', return_value=store), \
                    patch('autostart.Connection', return_value=connection):
                self.assertEqual(autostart.main(), 0)
                connection.start.assert_not_called()
                logger.info.assert_called_with(expected)
        connection = Mock(running_since=None)
        connection.events = queue.Queue()
        def start(*args):
            connection.error_kind = (ConnectionErrorKind.DOCTOR_FAILED if connection.start.call_count == 1
                                     else ConnectionErrorKind.AUTH_FAILED)
            connection.events.put(('done', ''))
        connection.start.side_effect = start
        with patch('tray_windows.kernel'), patch('autostart.ctypes.get_last_error', return_value=0), \
                patch('autostart.create_logger'), patch('autostart.load_settings', return_value=settings), \
                patch('autostart.build_commands'), patch('autostart.KeyStore') as store, \
                patch('autostart.Connection', return_value=connection), patch('autostart.time.sleep') as sleep:
            store.return_value.load.return_value = 'fake-key'
            self.assertEqual(autostart.main(), 0)
            self.assertEqual(connection.start.call_count, 2)
