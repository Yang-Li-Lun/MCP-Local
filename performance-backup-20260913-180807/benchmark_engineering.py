...
def measure(call):
    counters = {'read_bytes': 0, 'stat_calls': 0, 'lstat_calls': 0}
    ...
    with patch.object(Path, 'open', opened), patch.object(Path, 'stat', stat), patch.object(Path, 'lstat', lstat):
        result = call()
...
def run(source: Path, repeats=3):
    ...
    with tempfile.TemporaryDirectory(prefix='mcp-engineering-bench-') as directory:
        ...
        report = {'fixture_version': 1, 'repeats': repeats, 'instrumentation': 'tracemalloc and Python stat/read hooks; OS cache not flushed', 'samples': []}
        for _ in range(repeats):
            ...
            _, search = measure(lambda: reader.search_text('marker', limit=1))
